...
async def main():
    ...
    if args.bootstrap:
        ip, port = args.bootstrap.split(":")
        bootstrap_address = (ip, int(port))
    ...
    gossip_task = asyncio.create_task(start_gossip_node(
        host=(args.host, args.port+1000),
        bootstrap=bootstrap_address,
        full_node=args.broadcast
    ))

    # Start gRPC and API in threads
    grpc_thread = threading.Thread(target=start_grpc_server, name="gRPC", daemon=True)
